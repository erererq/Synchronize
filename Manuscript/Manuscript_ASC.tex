...
\]
The drive is integrated for 1000 time units, with the first 50 discarded. The state and reduced variational equations use RK4 with renormalization by QR factorization at each step, where $Q$ is orthogonal and $R$ is upper triangular. Step sizes 0.01, 0.005, and 0.0025 give the following ranges:
\[
\begin{aligned}
...
\input{comparison_multi_input_ASC.tex}

\subsection{Controller Comparison and Robustness Assessment}
\label{subsec:robustness_tests}

Having examined the configuration choices, we now compare how different controllers use the same bounded input under nominal and perturbed dynamics. The assessment includes validated LQR and SMC settings, fixed-gain feedback, PPO, and SAC. We first examine LQR weight selection because it affects the error--effort ranking. The subsequent disturbance sweeps retain the original $R=1$ LQR curves as a reference, while the selected weighting and SAC results provide the broader comparison at the reported test conditions.

\input{comparison_lqr_sensitivity_ASC.tex}


We next examine how the frozen controllers respond as model uncertainty increases~\cite{zhang2011weak}. The drive dynamics remain unchanged, while uncertainty in the response system is introduced through its weight matrix:
\begin{equation}
...
The screening example gives this finding an operational interpretation. An accuracy requirement first determines which candidates remain feasible; observation dimension and energy then resolve the resource preference. Its mismatch tests show why a choice made under nominal conditions should be checked over the intended operating range. Adding actuators offers a different route to improved full-horizon accuracy, but also increases available control authority and measured total effort. Accuracy and applied effort can therefore be competing design requirements~\cite{rodriguez2020multiobjective}, even though the PPO reward used here optimizes error alone.

Controller choice adds a further layer to this tradeoff. LQR weighting changes the traditional-baseline ranking, and SAC produces different effort penalties despite reproducing the tested observation-subset error ordering. The resulting design lesson is to compare configurations against explicit accuracy and resource requirements, rather than selecting a controller from a single error score. These conclusions concern the stated three-state network, deployment observations, and separately tested perturbations. Extension to other networks, combined disturbances, and measured hardware costs provides the next step beyond this assessment.
