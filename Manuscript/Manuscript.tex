...
The central question is how actuator location changes the consequences of restricting feedback information under bounded single-input synchronization. A second question concerns the accuracy and applied-effort cost of using fewer control channels. PPO provides a common feedback-learning implementation for these controlled comparisons. Local transverse and linear-control indicators guide candidate screening; observed closed-loop behavior is then evaluated separately. State boundedness, trajectory sensitivity, and synchronization residuals are assessed separately to connect analytical properties with deployed performance.

The experiments connect each question to a corresponding comparison: node-dependent observation reduction examines feedback-information sensitivity, while all nonempty actuator subsets assess actuation-resource choices. Traditional controllers and matched-budget SAC determine how the findings depend on controller selection. The Node 2 comparison examines how nonlinear learned feedback and model-based designs handle the same bounded actuation channel. Error and applied effort are reported together.
...
\begin{itemize}
\item A fixed-dimension assessment covers all three paired observation subsets at Nodes 2 and 3, separating input size from coordinate composition in the tested Hopfield network. Changing the four-dimensional observation subset improves synchronization at Node 2 but worsens residuals and effort at Node 3 under the same memoryless learning setup. A targeted SAC check reproduces the direction of the error changes across the original and alternative subsets at both nodes. A screening example then ranks feasible candidates within a prespecified pool by feedback dimension and control effort, with choices checked on held-out initial conditions. This connects observation-subset performance with an explicit accuracy--resource selection criterion.
...
The complete actuator-subset comparison quantifies the cost of restricting actuation. Single-input Node 3 uses fewer actuators and less measured total effort than the favorable multi-input configurations, with a higher full-horizon error but not uniformly worse steady-state precision. Traditional-controller and SAC results reveal how controller design influences the balance between accuracy and effort across operating conditions. The boundedness analysis establishes non-divergence, while finite-time conditional Lyapunov estimates and multi-step checks characterize local perturbation attenuation along the evaluated trajectories.

The resulting contribution is a comparative assessment of actuator-dependent observation sensitivity and actuation-resource tradeoffs, supported by an explicit separation of analytical properties and numerical evidence. A targeted SAC cross-check reproduces the direction of the original-versus-alternative subset error changes at both nodes, although the effort penalties differ between algorithms. The selection example within a fixed candidate pool further links this assessment to an explicit resource preference and held-out initial-state checks, including no-selection outcomes. The study establishes these relationships for the specified network, tested policies, and observation subsets, using illustrative accuracy tolerances. The tested choices are insensitive to the examined pass-fraction levels but can change with resource priority, and their original accuracy requirements cease to hold at the larger tested mismatch amplitudes. Future work should extend the observation-subset comparison, test whether the placement dependence persists under changed network parameters, and develop certified synchronization-error bounds for sampled learned feedback.
