...
\begin{keywords}
Chaotic synchronization \sep
Pinning control \sep
Proximal policy optimization \sep
Partial observation \sep
...
To show component-level behavior, Fig.~\ref{fig:node_error_comparison} presents two-dimensional heat maps from the archived multi-initial-condition trajectories. Within each panel, rows represent test trajectories, time is horizontal, and color encodes signed synchronization error. Node 3 generally has a shorter transient than Node 2, whereas large fluctuations persist for Node 1. These qualitative observations agree with the performance ordering in Table~\ref{tab:pinning_node_metrics}; they are not additional independent training replications.
...
\centering\small
Password + decimal pixel sum\\
$\downarrow$ SHA-256 / floating-point seed\\
Logistic sequence $\longrightarrow$ byte mask $Q$\\[3pt]
Three-channel image $\longrightarrow$ ordered blocks\\
$\downarrow$ channelwise XOR with $Q$\\
Two rounds of block-indexed DNA substitution\\
$\uparrow$ four preprocessed drive streams\\
$\downarrow$ restore original block positions\\
Cipher image
\end{minipage}}
\captionof{figure}{Implemented image-transform pipeline. Response-side reconstruction reverses the DNA substitutions and XOR masking; no cross-channel diffusion is implemented.}
\label{fig:encrypt_process}
\end{minipage}
\end{center}
