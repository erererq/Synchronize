...
# 引入解耦的方案 1（唯一核心方案：3D 物理轨迹自适应归一化 + 互补双网格量化）
...
def generate_seed(
    ...
    **kwargs
    ...
    """
    统一序列生成入口（全面采用方案 1：连续混沌轨迹自适应归一化 + 互补双网格量化）。
    ...
    Returns:
        ...
        slave_sequence:  (S1, S2, S3, S4, S5, S6) uint8，值域 1~24。
        raw_arrays:      原始连续浮点轨迹。
    """
...
def _save_results(master_seq, slave_seq, raw_arrays, output_dir: str):
    """保存序列数据和对比图到实验目录。"""
    os.makedirs(output_dir, exist_ok=True)

    # 1. 保存量化后序列为 .npz
    npz_path = os.path.join(output_dir, "chaotic_sequences.npz")
    ...
    try:
        ...
        fig.suptitle("Master vs Slave Chaotic Sequences [6-Sequence Architecture]", fontsize=14)
        ...
        fig_path = os.path.join(output_dir, "sequence_comparison.png")
        ...
        fig2.suptitle("Synchronization Error (Master - Slave)", fontsize=14)
        ...
        err_path = os.path.join(output_dir, "sync_error.png")
...
def main():
    parser = argparse.ArgumentParser(description="混沌同步序列生成与调度实验 (方案 1 纯 3D 动力学)")
    ...
    print("=" * 60)
    print("混沌序列生成调度引擎 (方案 1：6 序列 3 轮不对称编解码)")
    ...
    for i in range(len(master_seq)):
        ...
        print(f"  序列 L{i+1}: 独立值计数 = {u_count} (值域 1~24)")
        assert u_count > 1, f"警告：序列 L{i+1} 发生了常数坍缩！"

    _save_results(master_seq, slave_seq, raw_arrays, args.output_dir)

    print("\n" + "=" * 60)
    print("方案 1 (6 序列架构) 序列生成与保存已成功完成！")
    print("=" * 60)
