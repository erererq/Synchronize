...
class FHNEnv2D(gymnasium.Env):
    """Drive-response synchronization task for a periodically forced 2D FHN neuron."""
    ...
    def reset(self, seed=None, options=None):
        ...
        self.current_step = 0
        self.time = 0.0
        self.statex = self.np_random.uniform(low=-0.8, high=0.8, size=2).astype(np.float32)
        self.statey = self.np_random.uniform(low=-0.8, high=0.8, size=2).astype(np.float32)
    ...
    def get_derivatives(
        ...
        dx = x - (x**3) / 3.0 - y + bias + forcing + control
        dy = eps * (x + a - b * y)
