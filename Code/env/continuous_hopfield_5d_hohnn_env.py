...
class ContinuousHopfield5DHOHNNEnv(gym.Env):
    """5D high-order Hopfield neural network synchronization environment.

    Identifier intent: 5D_HOHNN_LIU2024.
    Model form:
    ...
    def __init__(
        ...
        *,
        ...
        self.system_tag = "5D_HOHNN_LIU2024"
        self.dim = 5
        self.dt = 0.001
        self.rk4_steps = 10
        self.max_steps = 600
        ...
        self.w22 = float(w22)
        self.w44 = float(w44)
        self.c = np.array([1.0, 1.0, 1.0, 100.0, 2.0], dtype=np.float32)
        ...
        if not 0 <= int(pinning_node) < self.dim:
            raise ValueError(f"pinning_node must be in [0, {self.dim - 1}].")
        self.B[int(pinning_node)] = 1.0
    ...
    def _build_weight_matrix(self) -> np.ndarray:
        return np.array(
            [
                [self.w11, 0.5, -3.0, -1.0, -0.01],
                [0.1, self.w22, 3.0, -0.1, 0.01],
                [3.0, -3.0, 1.0, -1.0, 0.01],
                [100.0, -0.1, 0.1, self.w44, -1.0],
                [1.0, -0.1, 0.1, 2.0, 1.0],
            ],
            dtype=np.float32,
        )
