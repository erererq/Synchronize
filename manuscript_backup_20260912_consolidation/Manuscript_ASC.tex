...
\input{comparison_observation_identity_ASC.tex}

\input{comparison_sac_observation_ASC.tex}

\input{comparison_configuration_selection_ASC.tex}
...
At $\eta=2$, the selected LQR design gives Node 2 MAE 0.1486 with energy 14.60, compared with 0.1703 and 11.85 for PPO; SAC gives 0.1513 and 9.31 (Table~\ref{tab:sac_comparison}). SMC with the selected parameters gives MAE 0.2223, while fixed feedback with unchanged $k=8$ has large errors. Thus, the results at this mismatch level show that lower error can require greater control energy, depending on the controller. Fig.~\ref{fig:mismatch_methods} retains the reference $R=1$ LQR curve (MAE 0.2330 at this endpoint) to show how the original settings respond across mismatch amplitudes. Table~\ref{tab:traditional_summary} summarizes those reference settings. Node 3 generally shows smaller absolute error differences.
...
At $\sigma=0.05$, the selected LQR gives lower Node 2 mean MAE and energy (0.2427 and 90.27) than PPO (0.2669 and 135.57). SAC has a similar mean MAE to PPO (0.2632), with lower energy (86.13). SMC with the selected parameters gives MAE 0.3295. The comparison therefore depends on the LQR weighting: its reference $R=1$ design gives MAE 0.3742, while the selected weighting and SAC give lower error with different energy requirements. Fig.~\ref{fig:noise_methods} and Table~\ref{tab:noise_summary} retain the reference-controller noise sweep for both nodes. Noise is added to the evolving response state, so the experiment measures sensitivity to process noise.
